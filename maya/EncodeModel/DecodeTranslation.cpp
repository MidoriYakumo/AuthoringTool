...
MatrixXd DecodeTranslation( MatrixXd &features, MatrixXi &faces, MatrixXd &temp ){
	...
	for( int i = 0; i < faces.rows(); ++i ){
		...
		Vector3d p12 = ( x.col( 0 ) - x.col( 1 ) ).normalized();
		Vector3d p13 = ( x.col( 0 ) - x.col( 2 ) ).normalized();
		Vector3d p23 = ( x.col( 1 ) - x.col( 2 ) ).normalized();
		...
		for( int j = 0; j < 3; ++j ){
			dx.row( v[ j ] ) += tmp.row( j );
		}
	}

	//M.coeffRef( temp.rows(), 0 ) = 1;
	//dx...
